...
paciente_tag = Tag(name="Paciente", description="Adição, visualização, remoção e predição de pacientes com Cálculos Renais")
...
def predict(form: PacienteSchema):
    ...
    Arguments:
            ...
            grav(float): Gravidade específica, dendidade da urina em relação à água.
            ph(float): Logaritmo negativo do íon de hidrogênio.
            osm(int): Osmolaridade é proporcional à concentração de moléculas em solução.
            cond(float): Condutividade é proporcional à concentração de íons carregados na solução.
            urea(int): Concentração de ureia em milimetros por litro
            calc(float): Concentração de cálcio (CALC) em milimetros por litro.
    ...
    # Carregando modelo
    ml_path = 'ml_model/modelo_testado.pkl'
